// File: c:/bsd/rigel/sort/integersorters/C4.c
// Date: 2012 // Sat Jul 23 08:17:35 2022, Fri Dec 13 15:35:13 2024
...
static const int cut4Limit = 1000; // 1-pivot prefix bound

static void cut4c();
// cut4 is doing 4-partitioning using 3 pivots
void cut4(int *A, int lo, int hi) {
  // printf("cut4 %d %d \n", lo, hi);
  int L = hi - lo; 
  if ( L <= 0 ) return;
  if ( L < cut4Limit ) {
  ...
  cut4c(A, lo, hi, depthLimit);
} // end cut4

// long int cnt2 = 0; 

void cut4c(int *A, int lo, int hi, int depthLimit) {
  int L;
 Start:
  ...
  if 
    ...
    dflgm(A, lo, hi, middlex, cut4c, depthLimit);
    return;
  }
 ...
 TLgMLMRgRL:
	...
	A[i] = x;
	x = y;
	// if ( compareXY(minr, y) <= 0 ) {  goto TLgMLMRgRR; }
	if ( minr <= y ) goto TLgMLMRgRR;
	goto TLgMLMRgRMR;
 ...
 TLMLMRgRR:
	...
	if ( y <= middle) {
	  A[j] = x;
	  x = y;
	  // if ( compareXY(y, maxl) <= 0 ) goto TLMLMRgRL;
	  if ( y <= maxl ) goto TLMLMRgRL;
	  // y -> ML
	  goto TLMLMRgRML;
	}
	// middle < y
